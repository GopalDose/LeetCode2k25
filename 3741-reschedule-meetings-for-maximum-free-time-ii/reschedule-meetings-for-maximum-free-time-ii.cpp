...
public:
    int maxFreeTime(int eventTime, vector<int>& startTime, vector<int>& endTime) {
        ...
        // Find the original maximum free time
        long long orig = 0;
        for (long long gap : G) {
            orig = max(orig, gap);
        }
        ...
        // Try removing each event and compute max free time
        for (int i = 0; i < n; i++) {
            long long du = (long long)endTime[i] - startTime[i];  // Event duration
            long long dumm = 0, tp = 0;
